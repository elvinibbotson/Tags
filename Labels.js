var tab={}; // tab object
var folder=null; // Labels bookmarks folder ID
...
id('label').addEventListener('click',function() {
...
id('search').addEventListener('click',function() {
	...
	console.log('get bookmarks in Labels folder - id: '+folder);
...
// check for Labels bookmarks
var found=false;
chrome.bookmarks.search({title:'Labels'},
(results)=>{
	for(const result of results){
		console.log(result);
		if(result.title==='Labels') {
			found=true;
			folder=result.id;
		}
	}
	if(found) console.log('Labels bookmarks folder exists - id: '+folder);
	else makeFolder();
});
// populate with icons
var html;
...
function makeFolder() {
	chrome.bookmarks.create({title: 'Labels'},
